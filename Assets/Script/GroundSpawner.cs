using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class GroundSpawner : MonoBehaviour
{
    public GameObject [] Prefabs; // array for different prefabs taht can be spawned  play area
	private Transform Player;

	private List<GameObject> ActivePrefabs;


	public float BackArea = 200.0f; //The distance from the player at which new prefabs should start spawning.
	public int PrefabsOnScreen = 4;
	public int LastPrefab = 0;
	public float SpawnPrefab = -100.0f;
	public float PrefabLength = 99.0f;
    // Start is called before the first frame update
    void Start () 
    {
            ActivePrefabs = new List<GameObject> ();
            Player = GameObject.FindGameObjectWithTag ("Player").transform;

            for (int i = 0; i < PrefabsOnScreen; i++) 
            {
                if (i < 3)
                    Spawn (0);
                else
                    Spawn ();       
            }
    }   

    // Update is called once per frame
    private void Update () 
	{
		if (Player.position.z - BackArea > (SpawnPrefab - PrefabsOnScreen * PrefabLength)) 
		{
			Spawn ();
			DeletePrefab ();
		}
	}


	private void Spawn (int prefabIndex = -1)
	{
		GameObject myPrefab;
		if (prefabIndex == -1)

			myPrefab = Instantiate (Prefabs [RandomPrefabs ()] as GameObject);
		else
			myPrefab = Instantiate (Prefabs [prefabIndex] as GameObject);

		myPrefab.transform.SetParent (transform); //The instantiated prefab is set as a child of the current game object

		myPrefab.transform.position = Vector3.forward * SpawnPrefab; //The prefab's position is set to a specific z-position (SpawnPrefab) along the forward axis.
		SpawnPrefab += PrefabLength;
		ActivePrefabs.Add (myPrefab);
	}

	private void DeletePrefab ()
	{
		Destroy (ActivePrefabs [0]); //It destroys the oldest prefab in the ActivePrefabs list using Destroy
		ActivePrefabs.RemoveAt (0); //The prefab is also removed from the list using RemoveAt
	}


	private int RandomPrefabs ()
	{
		if (Prefabs.Length <= 1)
			return 0;
		int randomIndex = LastPrefab;
		while (randomIndex == LastPrefab) 
		{
			randomIndex = Random.Range (0, Prefabs.Length);
		}

		LastPrefab = randomIndex;
		return randomIndex;
	}
}
